...
async def validate_image(image: UploadFile) -> Tuple[Optional[bytes], Optional[JSONResponse]]:
    """
    ...
    if len(image_bytes) > max_bytes:
        return None, JSONResponse(
            status_code=413, 
            content={"detail": f"File exceeds maximum allowed size of {max_mb}MB", "code": "INVALID_FILE"}
        )
    ...
    if mime_type not in ["image/jpeg", "image/png", "image/webp"]:
        return None, JSONResponse(
            status_code=400, 
            content={"detail": f"Unsupported file format ({mime_type}). Only JPEG, PNG, and WebP are allowed.", "code": "INVALID_FILE"}
        )
    ...
    try:
        ...
        return None, JSONResponse(
            status_code=400, 
            content={"detail": "File is corrupted or not a valid executable image matrix.", "code": "INVALID_FILE"}
        )
