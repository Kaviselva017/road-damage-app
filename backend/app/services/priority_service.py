...
def calculate_priority_score(damage_type: str, severity: str, confidence: float, area_type: str, nearby_sensitive: str, report_count: int, latitude: float, longitude: float, weather_risk: float, db: Session) -> PriorityResult:
    """
    ...
    damage_bases = {"pothole": 15, "crack": 8, "subsidence": 25, "flooding": 20, "debris": 10}
    ...
    if "hospital" in nearby:
        area_bonus = max(area_bonus, 20)
    elif "school" in nearby:
        area_bonus = max(area_bonus, 15)
    elif "highway" in area_t:
        area_bonus = max(area_bonus, 18)
    elif "commercial" in area_t:
        area_bonus = max(area_bonus, 8)
    elif "residential" in area_t:
        area_bonus = max(area_bonus, 5)
    ...
    total_score = min(factors["damage_base"] + factors["severity_bonus"] + factors["area_bonus"] + factors["volume_bonus"] + factors["ai_confidence_factor"] + factors["weather_risk_factor"], 100)
    ...
    return {"score": round(total_score, 1), "urgency_label": label, "factors": factors, "recommended_sla_hours": sla}
