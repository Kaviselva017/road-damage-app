...
def cluster_complaints(db: Session, grid_size_meters: int = 500) -> List[ClusterPoint]:
    ...
    if is_postgres:
        ...
        return [
            ClusterPoint(
                lat=r.lat, lng=r.lng, count=r.count, weight=float(r.total_weight),
                dominant_damage_type=r.dominant_type or "pothole",
        ...
        for c in complaints:
            ...
            if key not in clusters:
                clusters[key] = {
                    "count": 0, "types": {}, "severities": [], "total_weight": 0.0,
                    "lat": key[0], "lng": key[1]
                }
        ...
        for key, data in clusters.items():
            ...
            final.append(ClusterPoint(
                lat=data["lat"], lng=data["lng"], count=data["count"],
                weight=data["total_weight"], dominant_damage_type=dominant,
                max_severity=max_s
            ))
        return final
