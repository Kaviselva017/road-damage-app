...
if SMTP_USER and SMTP_PASS:
    ...
    logger.warning("[Email] No email provider configured — emails disabled. Add SMTP_USER + SMTP_PASS to Render env vars.")
...
def _resend(to: str, subject: str, html: str) -> bool:
    ...
    try:
        ...
        if e.code == 422:
            logger.error("[Email Resend] ⚠ 422 = domain not verified. Use Gmail SMTP instead: add SMTP_USER + SMTP_PASS.")
        return False
    except Exception as e:
...
def _row(label: str, value: str, color: str = "#e8eaf0") -> str:
    return f'<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #1e2535"><span style="color:#6b7694;font-size:13px">{label}</span><span style="color:{color};font-size:13px;font-weight:600">{value}</span></div>'
...
def _base(title: str, body: str, cta_url: str = "", cta_text: str = "", image_url: str = "", header_color: str = "#00e5ff") -> str:
    ...
    cta = (f'<div style="text-align:center;margin:30px 0"><a href="{cta_url}" style="background:#f5a623;color:#000;padding:12px 30px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block">{cta_text} →</a></div>') if cta_url else ""

    img = (f'<div style="margin-bottom:20px"><img src="{image_url}" style="width:100%;max-height:300px;object-fit:cover;border-radius:12px;border:1px solid rgba(255,255,255,0.1)" /></div>') if image_url else ""
...
def notify_welcome(to_email: str, citizen_name: str) -> bool:
    body = (
        f'<p style="color:#9ca3af;margin:0 0 16px">Hi <strong style="color:#e8eaf0">'
    ...
    return send_email(to_email, "Welcome to RoadWatch!", _base("Account Created!", body, f"{BASE_URL}/citizen", "Start Reporting"))
...
def notify_complaint_submitted(
    ...
    if nearby_places:
        ...
        nearby_section = (
            f'<div style="margin-top:20px;padding:15px;background:#1e293b;border-radius:10px;border:1px solid rgba(255,255,255,0.05)">'
    ...
    body = (
        f'<p style="color:#f1f5f9;font-size:16px;margin:0 0 15px">Hi <strong>{citizen_name}</strong>,</p>'
        ...
        f"{_row('Area', area_type.replace('_', ' ').title(), '#f5a623')}"
    ...
    return send_email(to_email, f"Complaint Registered: {complaint_id}", _base("Complaint Registered", body, f"{BASE_URL}/citizen", "Track Your Complaint", image_url=image_url, header_color="#10b981"))
...
def notify_officer_assignment(
    ...
    body = f'<p style="color:#f1f5f9;font-size:16px;margin:0 0 15px">Officer <strong>{officer_name}</strong>,</p><p style="color:#94a3b8;line-height:1.5;margin-bottom:20px">A new road damage complaint has been assigned to you for inspection and resolution.'
    ...
    if notes:
        body += f'<div style="margin-top:20px;background:rgba(59,130,246,0.1);padding:15px;border-radius:10px;border-left:4px solid #3b82f6"><div style="font-size:13px;font-weight:700;color:#3b82f6;margin-bottom:5px">ℹ AI Analysis Insight</div><div style="font-size:13px;color:#94a3b8">{notes}</div></div>'

    return send_email(to_email, f"New Assignment: {complaint_id}", _base("Complaint Assigned to You", body, f"{BASE_URL}/dashboard", "Open Officer Dashboard", image_url=image_url, header_color="#f5a623"))
...
def notify_status_update(
    ...
    if officer_notes:
        ...
        notes_html = f'<div style="background:rgba(59,130,246,0.08);border:1px solid rgba(59,130,246,0.3);border-radius:8px;padding:10px;margin:12px 0"><div style="font-size:11px;color:#60a5fa;font-weight:700;margin-bottom:4px">Officer Notes{by}</div><div style="font-size:13px">{officer_notes}</div></div>'
    body = (
        f'<p style="color:#9ca3af;margin:0 0 14px">Hi <strong style="color:#e8eaf0">'
    ...
    return send_email(to_email, f"RoadWatch Update: {complaint_id} - {label}", _base("Status Updated", body, f"{BASE_URL}/citizen", "View Complaint", image_url=image_url))
...
def notify_fund_allocated(
    ...
    return send_email(to_email, f"RoadWatch: Budget Allocated for {complaint_id}", _base("Budget Allocated 💰", body, f"{BASE_URL}/citizen", "Track Progress"))
...
def notify_admin_emergency(
    ...
    body = (
        f'<div style="background:rgba(224,92,92,0.12);border:1px solid #e05c5c;'
    ...
    return send_email(ADMIN_EMAIL, f"URGENT: High Severity {complaint_id} - RoadWatch", _base("Emergency Alert", body, maps, "View on Maps", image_url=image_url))
