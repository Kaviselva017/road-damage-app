...
from app.models.models import Complaint, SLAConfig, Department, Notification
...
def calculate_deadline(db: Session, severity: str) -> Optional[datetime]:
...
def get_department_for_damage(db: Session, damage_type: str, area_type: str) -> Optional[int]:
    ...
    for c in overdue:
        ...
        if now > (last_mark + timedelta(hours=esc_hours)):
            ...
            db.add(Notification(
                ...
                message=f"Urgent: SLA for {c.complaint_id} breached. Escalated to {level_name}.",
                type="escalation",
                created_at=now
            ))
            ...
            audit_service.log_event(
                db, "complaint", c.complaint_id, "escalated",
                actor_id=None, actor_role="system",
                old_value={"level": c.escalation_level - 1},
                new_value={"level": c.escalation_level}
            )
                ...
                try:
                    ...
                    if user and user.fcm_token:
                        ...
                        asyncio.create_task(send_push(
                            ...
                            body=f"Your report {c.complaint_id} has been escalated to {level_name} for priority resolution.",
                            data={"complaint_id": c.complaint_id, "screen": "complaint_detail", "status": "escalated"}
                        ))
