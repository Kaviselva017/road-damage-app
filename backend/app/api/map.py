...
async def get_heatmap(
    grid: int = Query(500, ge=100, le=2000),
...
async def get_map_timeline(
    ...
    results = db.query(
        ...
        func.sum(func.case((Complaint.severity == 'high', 1), else_=0)).label("high_sev")
    ).filter(Complaint.created_at >= start_date).group_by(func.date(Complaint.created_at)).order_by("date").all()
