...
def get_admin(db: Session = Depends(get_db), current_officer: FieldOfficer = Depends(get_current_officer)):
    """Any logged-in officer can access admin — no special role check"""
...
def get_all_complaints(
    ...
    for c in complaints:
        ...
        for field in ["allocated_fund", "fund_note", "priority_score", "rainfall_mm", "traffic_volume", "road_age_years", "weather_condition"]:
            try:
...
async def update_officer(officer_id: int, request: Request, db: Session = Depends(get_db), admin=Depends(get_admin)):
...
def delete_officer(officer_id: int, db: Session = Depends(get_db), admin=Depends(get_admin)):
    o = db.query(FieldOfficer).filter(FieldOfficer.id == officer_id).first()
    if not o:
        raise HTTPException(status_code=404, detail="Officer not found")
...
async def reassign_complaint(complaint_id: str, request: Request, db: Session = Depends(get_db), admin=Depends(get_admin)):
    body = await request.json()
    ...
    try:
        ...
        if officer:
            notify_officer_assigned(officer, c)
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"Officer notification error: {e}")
    
    return {"message": "Complaint reassigned", "officer_notified": True}

@router.patch("/citizens/{user_id}/block")
async def toggle_citizen(user_id: int, request: Request, db: Session = Depends(get_db), admin=Depends(get_admin)):
    body = await request.json()
