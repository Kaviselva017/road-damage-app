...
from app.models.models import Complaint, FieldOfficer, Notification
...
def run_audit():
    ...
    try:
        ...
        if stale:
            ...
            # Notify Admin
            admin = db.query(FieldOfficer).filter(FieldOfficer.is_admin == True).first()
            if admin:
                subject = f"Audit Alert: {len(stale)} Complaints require attention"
                body = f"<h2>Audit Report</h2><p>The following complaints have been pending/assigned for more than 48 hours without progress:</p><ul>"
                for c in stale:
